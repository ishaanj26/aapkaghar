...
export default function UpdateListing() {
    ...
    const handleImageSubmit = (e) => {
        if (files.length > 0 && formData.images.length + files.length < 7) {
            setUploading(true);
    ...
    const handleChange = (e) => {
        if (e.target.id === 'sale' || e.target.id === 'rent') {
            setFormData({
                ...formData,
                type: e.target.id, //will change type to sale or rent
            });
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.post(`http://localhost:3000/api/listing/update/${params.listingId}`, { ...formData, userRef: userData._id })
            if (data.success) {
                setLoading(false);
    ...
    return (
        <main className='p-6 bg-gray-100 mt-5  max-w-4xl mx-auto'>
            <h3 className="text-2xl font-bold mb-4">Update a List</h3>
